...
async function main() {
  console.log('🌱 Seeding products manually...')
  ...
  // INSERT SATU PER SATU (paling aman)
  for (let i = 0; i < products.length; i++) {
    try {
      await prisma.product.create({
        data: products[i]
      })
      console.log(`✅ Created (${i+1}/${products.length}): ${products[i].name}`)
    } catch (error) {
      console.error(`❌ Failed to create ${products[i].name}:`, error)
    }
  }
